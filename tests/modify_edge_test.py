...
def test_api(
  ...
  modify_edge = {"edges": [{"source": "Sea Level Pressure Tahiti", "target": "Southern Oscillation Index SOI", "polarity": 1.0, "weights": [0.0, 0.0]}]}
  _ = requests.post(
    f"{service}/models/{cag_id}/edit-edges",
    auth = auth,
    data = json.dumps(modify_edge)
  )

  modify_edge = {"edges": [{"source": "Sea Level Pressure Darwin", "target": "Southern Oscillation Index SOI", "polarity": -1.0, "weights": [0.0, 0.9]}]}
  _ = requests.post(
    f"{service}/models/{cag_id}/edit-edges",
    auth = auth,
    data = json.dumps(modify_edge)
  ...
  create_json = requests.get(f"{service}/models/{cag_id}").json()
  print(create_json)
